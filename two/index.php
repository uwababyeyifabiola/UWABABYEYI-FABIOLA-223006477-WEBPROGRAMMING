...
<body>
    <h1 style="text-align: center; text-transform: uppercase;">Student Registration Form</h1>
    <?php if ($message): ?>
    ...
    <form method="post" action="<?= htmlspecialchars($_SERVER['PHP_SELF']) ?>">
        ...
        <div class="row">
            <label>Gender</label>
            <label class="inline"><input value="Male" type="radio" name="gender" <?= isset($posted['gender']) && $posted['gender'] === 'Male' ? ' checked' : '' ?>> Male</label>
        ...
        <div>
            ...
            <?php foreach ($hobbyOptions as $hobby): ?>
                <label class="inline"  style="text-transform:capitalize;"><input style="margin-left: 5px;" type="checkbox" name="hobbies[]" value="<?= $hobby ?>"<?= checked($posted['hobbies'] ?? [], $hobby) ?>> <?= $hobby ?></label>
            <?php endforeach; ?>
            <div class="row">
                ...
                <tbody>
                    ...
                    <?php $index = 1; foreach ($qualRows as $label => $fieldPrefix): ?>
                        <tr>
                            ...
                            <td><inputtype="text" name="<?= $fieldPrefix ?>_year" value="<?= sanitize($posted[$fieldPrefix . '_year'] ?? '') ?>" maxlength="4"></td>
                        </tr>
                    <?php endforeach; ?>
                    
                </tbody>
            </table>
            <p> (10 char max)     ( up to 2 decimal)</p>
        </div>
